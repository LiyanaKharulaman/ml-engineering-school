...
def train_pipeline(
    ...
    This function takes the training data along with LightGBM parameters, initializes a pipeline with preprocessing,
    and fits a LightGBM classifier to the training data.

    Args:
        x_train (pd.DataFrame): The training input samples.
        y_train (pd.DataFrame): The target values (class labels) as a DataFrame.
        model_objective (str): The objective function of the LightGBM model.
